...
<script type="text/javascript">
	...
	function memberDeptRefSet(gubun){
		...
		var refTy = container[0].children;
		var cont = $("#" + f + "DeptId");
		cont.empty();
		cont = $("#" + f + "DeptCode");
		cont.empty();
		...
		cont = $("#" + f + "DeptId");
		cont.append(refTy[0].outerHTML);

		var dept = refTy[0].attributes;
		cont = $("#" + f + "DeptCode");
		cont.append("<div class='radio btn_delete_img' style='margin-right:15px;padding-top:3px;padding-bottom:3px;' deptCode='"+dept.deptCode.value+"' deptNm='"+dept.deptNm.value+"'>"
		+ "<pre class='hidden deptInfo'>"+dept.deptNm.value+"</pre>"
		+ "<input type='radio' checked=''>"
		+ "<label>"
		+ "<span class='txt'>"+dept.deptNm.value+"</span>"
		+ "</label>"
		+ "</div>");
		
// 여러명 설정		
// 		var count = refTy.length;
// 		for(var i = 0; i < count; i++){
// 			if(refTy[i].attributes[4].value == 1){
// 				cont = $("#" + f + "DeptId");
// 				cont.append(refTy[i].outerHTML);//1 : 담당자, 2 : 부서 
// 			}
// 			else if(refTy[i].attributes[4].value == 2){
// 				cont = $("#" + f + "DeptCode");
// 				cont.append(refTy[i].outerHTML);//1 : 담당자, 2 : 부서 
// 			}
// 		}//for
