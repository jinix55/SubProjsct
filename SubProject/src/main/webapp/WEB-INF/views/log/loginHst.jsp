...
<section class="content">
	<div class="content_tit">
		<div><span class="content_h1">로그인 이력 관리</span></div>
	...
	<form name="logForm" id="logForm" method="get" action="/admin/loginHst">
			...
			<tbody id="loginHstBody">
				<c:forEach var="log" items="${logs}" varStatus="status">
					<tr <c:if test="${log.userId eq 'admin'}">class="log_old"</c:if>
						...
						<c:if test="${log.methodNm eq 'onLogoutSuccess'}">
							<td>로그아웃</td>
						</c:if>
						<c:if test="${log.methodNm eq 'onAuthenticationSuccess'}">
							<td>로그인</td>
						</c:if>
						...
						<td>
							<spring:eval expression="log.logDt" />
						</td>
				</c:forEach>
			</tbody>
		</table>
