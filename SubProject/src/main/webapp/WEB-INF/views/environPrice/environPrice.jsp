...
<div class="content">
	<!-- S_검색-->
	<form id="searchFrm" action="/system/environPrice" method="POST">
...
<script type="text/javascript">
	...
	function viewCode(groupCd, groupNm, groupDesc, groupUseYn) {
		$.getJSON("/system/environPrice/detail/" + groupCd, function(codes) {
	...
	function insertAjax(param,action){
		$.ajax({
		    type : 'post',
		    url : '/system/environPrice/'+action,
		    data : param,
		    dataType : 'text',
		    ...
		    success : function(result){
		    	if(result == 'Update' || result == 'Insert'){
		    		location.href = '/system/environPrice';
		    	}
		    }
		});
	}
	...
	function upCodeList(id,nm){
		$.ajax({
			url : '/system/environPrice/detail/environPrice/'+id,
			dataType : 'json',
			type : "POST",
			async: false,
	...
	function makeUpCodeList(data,id,nm){
		...
		if(data.length > 0){
			data.forEach(function(item, index) {
				var ordSeq = '';
				var codeDsc = '';
				html += '<tr onclick="selectTr(\''+index+'\');">';
				html += '	<td>'+(index+1)+'</td>';
	...
	function makeInputList(data,groupId,grpNm){
		...
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">단위</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="unit" name="unit" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">재활용 의무율</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="recyclDutyRate" name="recyclDutyRate" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">단가(원)</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="unitPrice" name="unitPrice" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">할증</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="premium" name="premium" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">할인(%)</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="discount" name="discount" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
		html += '	<div class="col-50">';
		html += '		<div class="form-group">';
		html += '			<label class="col-25 form-label">기타</label>';
		html += '			<div class="col-75">';
		html += '				<div class="form-input">';
		html += '					<input id="other1" name="other1" type="text" class="text-input" value="">';
		html += '				</div>';
		html += '			</div>';
		html += '		</div>';
		html += '	</div>';
	...
	function selectTr(no){
		var data = setData[no];
		setCodeId = data.codeId;
		setGrpId = data.groupId;
		var ordSeq = '';
		var codeDsc = '';
		var useYn = '';
		if(data.ordSeq == 'null'){
		...
		if(data.codeDsc == 'null'){
			codeDsc = '';
		}else{
			codeDsc = data.codeDsc;
		}
		if(data.useYn == 'Y'){
			useYn = 'Y';
		}else{
		...
		$('#tr_'+data.groupId+'_input #groupId').val(data.groupId);
		$('#tr_'+data.groupId+'_input #codeId').val(data.codeId);
		$('#tr_'+data.groupId+'_input #codeNm').val(data.codeNm);
		$('#tr_'+data.groupId+'_input #code_use'+useYn).prop('checked',true);
		$('#tr_'+data.groupId+'_input #codeDsc').val(codeDsc);
		$('#tr_'+data.groupId+'_input #ordSeq').val(ordSeq);
		
		$('#tr_'+data.groupId+'_input #unit').val(data.unit);
		$('#tr_'+data.groupId+'_input #recyclDutyRate').val(data.recyclDutyRate);
		$('#tr_'+data.groupId+'_input #unitPrice').val(data.unitPrice);
		$('#tr_'+data.groupId+'_input #premium').val(data.premium);
		$('#tr_'+data.groupId+'_input #discount').val(data.discount);
		$('#tr_'+data.groupId+'_input #other1').val(data.other1);
		$('#insertBtn').show();
		$('#deleteBtn').show();
	}
	...
	function deleteBtn(groupId){
		...
		$.ajax({
		    type : 'post',
		    url : '/system/environPrice/delete/price',
		    data : param,
		    dataType : 'text',
		    error: function(xhr, status, error){
		        console.log(error);
	...
	function detailView(id){
		...
		$.ajax({
			url : '/system/environPrice/detail/'+id,
			dataType : 'json',
			type : "POST",
			async: false,
			success : function(data) {
	...
	function deleteAjax(){
		...
		$.ajax({
		    type : 'post',
		    url : '/system/environPrice/delete',
		    data : param,
		    dataType : 'text',
		    error: function(xhr, status, error){
		        console.log(error);
